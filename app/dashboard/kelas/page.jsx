...
const Page = () => {
  ...
  const userProfile = JSON.parse(Cookies.get('user_profile'));
  ...
  const getStudents = async () => {
    try {
      const res = await apiV1.get(
        `/teacher/classes/students${selectedClass ? `?class_id=${selectedClass.id}` : ''}`,
        {
          headers: {
            Authorization: `Bearer ${tokenServices.getAccessToken()}`,
          },
  ...
  const handleIconClick = (index, iconType) => {
    if (iconType === 'progressIcon') {
      router.push('/dashboard/kelas/progress');
    } else if (iconType === 'resultIcon') {
      router.push('/dashboard/kelas/hasil');
    ...
    <div className="w-screen h-screen flex">
      ...
      {userProfile.is_verified ? (
        <div className="w-4/5 px-[5vh] py-[4vh] flex flex-col gap-12">
          <button
            className="rounded px-[8px] py-[10px] w-fit bg-[#29ADB2]"
        ...
        <div className="w-4/5 h-full flex items-center justify-center">
          <p className="text-lg text-slate-500">
            Anda belum diverifikasi admin. Silahka hubungi admin terlebih dahulu.
          </p>
        </div>
      )}
    </div>
