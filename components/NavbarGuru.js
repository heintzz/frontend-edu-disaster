    ...
    return (
        <div className='w-1/5 h-screen bg-[#253333] flex flex-col items-center'>
            ...
            <div className='h-4/5 w-full flex flex-col items-center justify-between py-[4vh] px-[3vw]'>
                ...
                <button className={`w-full flex justify-between items-center p-3 rounded ${!isStatistikClicked ? 'bg-[#29ADB2]' : ''}`} onClick={handleKelasClick}>
                    <div className='flex gap-3 items-center'>
                        <FaHome size={16} color={"white"} />
                        <p className="font-kumbh font-semibold text-sm text-white">Kelas</p>
                    </div>
                    {!isStatistikClicked && <MdKeyboardArrowRight size={16} color={!isStatistikClicked ? 'white' : 'transparent'}/>}
                </button>
                <button className={`w-full flex justify-between items-center p-3 rounded ${isStatistikClicked ? 'bg-[#29ADB2]' : ''}`} onClick={handleStatistikClick}>
                    <div className='flex gap-3 items-center'>
