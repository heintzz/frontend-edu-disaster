...
export default function DisasterCard({ index, imageSrc, title, description, onClick }) {
  return (
    <div className="relative h-[480px] cursor-pointer" onClick={onClick}>
      <div className="h-full bg-black flex flex-col rounded-3xl relative z-[3]">
        ...
        >
          <div className="h-10 w-10 bg-[#D9D9D9] absolute left-1/2 -translate-x-1/2 grid place-content-center rotate-45 -bottom-5">
            <span className="-rotate-45 font-otomanopee text-[#A65526] text-3xl font-semibold">
              {index}
            </span>
          </div>
        </div>
        <div className="h-[40%] flex flex-col gap-y-1 items-center justify-center w-full relative z-[2] bg-black text-white rounded-b-3xl text-center px-6 py-2">
          <p className="font-otomanopee text-3xl font-semibold">{title}</p>
